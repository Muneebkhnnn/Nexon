...
function ReviewResume() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (!input) {
        return toast.error("Please upload first");
      }
      ...
      const response = await axios.post('/api/v1/ai/resume-review',
        formdata,
        {
          headers: {
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.files[0])} accept='application/pdf' type="file" className='w-full cursor-pointer  text-gray-600 p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' />
